...
async def on_ready(): # 봇이 실행되면 한 번 실행됨
    print("봇 실행 완료")
    await client.change_presence(status=discord.Status.online, activity=discord.Game("주식 가격 확인"))
...
async def on_message(message):
    ...
    if message.content == "주식가격확인":
        ch = client.get_channel(1315348409418252329)
        sending_message = f"삼성전자 주식 매수가 : {bid}, 매도가 : {offer}"
        print_message(sending_message)
        await send_message(ch, sending_message)

def print_message(sending_message):
    print(sending_message)

async def send_message(ch, sending_message):
    await ch.send(sending_message)
